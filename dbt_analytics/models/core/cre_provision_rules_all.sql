WITH CTE_PROVISION_RULES AS (
	SELECT
		...
		,TRY_CAST(PROPORTIONAL_PROVISION ->> 'target' AS INTEGER) AS PROPORTIONAL_PROVISION_TARGET_VALUE
		,TRY_CAST(PROPORTIONAL_PROVISION ->> 'unit' AS VARCHAR) AS PROPORTIONAL_PROVISION_TARGET_UNIT
		,TRY_CAST(PROPORTIONAL_PROVISION ->> 'value' AS FLOAT) AS PROPORTIONAL_PROVISION_VALUE
	FROM
		{{ ref('stg_provision_rules_all') }}
	QUALIFY
