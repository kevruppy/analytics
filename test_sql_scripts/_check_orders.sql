...
SELECT *, CASE WHEN RN> PROP_PROV_TGT THEN PROP_PROV_VAL ELSE 0 END AS GROSS_PROP_PROV, BASE_PROVISION AS GROSS_BASE_PROV, CASE WHEN 
IS_PLACEMENT THEN PLACEMENT_PROVISION ELSE 0 END AS GROSS_PLC_PROV
FROM CTE_ENRICHED
)
SELECT DATE_TRUNC('MONTH',CREATION_DATE), PRODUCT_NAME, sum(GROSS_BASE_PROV), sum(GROSS_PLC_PROV), sum(GROSS_PROP_PROV), count(*)
FROM X 
GROUP BY ALL
ORDER BY 1,2
...
WHERE PRODUCT_NAME = '.COM'
AND DATE_TRUNC('MONTH',CREATION_DATE)='2022-01-01'