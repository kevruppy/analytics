WITH CTE_ORD_GROSS_PROV AS (
	SELECT
		ORD.ORDER_ID
		,ORD.IS_CONFIRMED
		,PROV_BASE.BASE_PROVISION_VALUE AS GROSS_BASE_PROVISION_VALUE
		,COALESCE(PROV_PLACE.PLACEMENT_PROVISION_VALUE,0) AS GROSS_PLACEMENT_PROVISION_VALUE
	FROM
		{{ ref('int_orders_status_dates') }} AS ORD
	-- ALL ORDERS MUST HAVE A BASE COMMISSION
	INNER JOIN
		{{ ref('cre_provision_rules_base') }} AS PROV_BASE
		ON
			ORD.PRODUCT_NAME = PROV_BASE.PRODUCT_NAME
			AND
			ORD.CREATION_DATE BETWEEN PROV_BASE.START_DATE AND PROV_BASE.END_DATE
	-- ORDERS MIGHT HAVE A PLACEMENT PROVISION
	LEFT OUTER JOIN
		{{ ref('cre_provision_rules_placement') }} AS PROV_PLACE
		ON
			ORD.PRODUCT_NAME = PROV_PLACE.PRODUCT_NAME
			AND
			ORD.CREATION_DATE BETWEEN PROV_PLACE.START_DATE AND PROV_PLACE.END_DATE
)

,CTE_ORD_GROSS_NET_PROV AS (
	SELECT
		ORDER_ID
		-- GROSS
		,GROSS_BASE_PROVISION_VALUE
		,GROSS_PLACEMENT_PROVISION_VALUE
		-- NET
		,CASE
			WHEN IS_CONFIRMED = TRUE THEN GROSS_BASE_PROVISION_VALUE ELSE 0
		END AS NET_BASE_PROVISION_VALUE
		,CASE
			WHEN IS_CONFIRMED = TRUE THEN GROSS_PLACEMENT_PROVISION_VALUE ELSE 0
		END AS NET_GROSS_PLACEMENT_PROVISION_VALUE
	FROM
		CTE_ORD_GROSS_PROV
)

SELECT
	ORDER_ID
	,GROSS_BASE_PROVISION_VALUE
	,GROSS_PLACEMENT_PROVISION_VALUE
	,NET_BASE_PROVISION_VALUE
	,NET_GROSS_PLACEMENT_PROVISION_VALUE
FROM
	CTE_ORD_GROSS_NET_PROV
